...
// CONFIGURAÇÃO
const API_URL = 'http://localhost:3000' // Altere para sua URL de produção se necessário
const CONCORRENTE_ID = process.argv[2] || '1' // Passe o ID como argumento: node test-lord-completo.js 1

async function testarLordCompleto() {
  console.log('🧪 TESTE COMPLETO: Extrair e enviar produtos do Lord para a API\n')
  ...
  try {
    ...
    // Extrair dados dos primeiros 5 produtos
    const LIMITE = 5
    ...
    for (const produto of produtosExtraidos) {
      try {
        ...
        if (response.ok) {
          ...
          console.log(`   ❌ ${produto.nome}: ${error}`)
        }
      } catch (error) {
        erros++
